...
describe("Either", () => {
  describe("Ok", () => {
    ...
    it("Should map", () => {
      expect(good.map((right) => `${right} Morning!`)).toEqual(
        Ok.of("Good Morning!")
      );
    });

    it("Should not map error", () => {
      expect(good.mapErr((err) => new Error(err.message + "!"))).toEqual(
        Ok.of("Good")
      );
    });
  ...
  describe("Error", () => {
    ...
    it("Should map error", () => {
      expect(error.mapErr((err) => new Error(err.message + "!"))).toEqual(
        Err.of(new Error("Something is wrong!"))
      );
    });
